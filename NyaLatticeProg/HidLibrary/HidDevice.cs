            ...
            if (_deviceCapabilities.InputReportByteLength > 0)
            {
                ...
                if (_deviceReadMode == DeviceMode.Overlapped)
                {
                    ...
                    try
                    {
                        ...
                        if (bytesRead > 0) Marshal.Copy(nonManagedBuffer, buffer, 0, (int)bytesRead);
                    }
                    catch(Exception E) { Debug.WriteLine(E.Message); status = HidDeviceData.ReadStatus.ReadError; }
                    finally {
